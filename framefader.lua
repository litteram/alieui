...
function aLib:StartFadeIn(frame)
...
function aLib:StartFadeOut(frame)
...
local function FrameHandler(frame)
  if IsMouseOverFrame(frame) then
    aLib:StartFadeIn(frame)
  else
    aLib:StartFadeOut(frame)
  end
end
...
function aLib:CreateFrameFader(frame, faderConfig)
...
function aLib:CreateButtonFrameFader(frame, buttonList, faderConfig)
  aLib:CreateFrameFader(frame, faderConfig)
  if faderConfig.trigger and faderConfig.trigger == "OnShow" then
    return
  end
